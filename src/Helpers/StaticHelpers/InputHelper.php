...
use Modules\Core\Utilities\FormBuilder\FormInputs\AutocompleteInput;
use Modules\Core\Utilities\FormBuilder\FormInputs\HiddenInput;
use Modules\Core\Utilities\FormBuilder\FormInputs\SelectInput;
use Modules\Core\Utilities\FormBuilder\FormInputValidators\RequiredFormInputValidator;

class InputHelper
{

    public static function getKwArray()
    {
        $powerOptions = [
            ''   => '',
            4    => '4kW (5KS)',
    ...
    public static function getWarehouseChangeReasons()
    {
        return [
            1 => _i("Products expired"),
            2 => _i("Producs lost"),
            3 => _i("Products stolen"),
        ];
    }

    public static function getReceiptWidths()
    {

        return ["" => _i("Not selected"), '58mm' => '58mm', '70mm' => '70mm'];

    }

    public static function getShopServiceTypes()
    {
        return [
            1 => _i("Shop service"),
            2 => _i("Self service"),
            3 => _i("Service & Self service"),
        ];
    }

    public static function getCoreStockTypes()
    {
        return [
            1 => _i("Update stock status"),
            2 => _i("Update stock status and notify"),
            3 => _i("Do not update stock status"),
        ];
    }

    public static function getShopServiceTypesSelect($model)
    {

        $input = new SelectInput('web_shops', 'web_shop_service_type_id', $model->web_shop_service_type_id ?? 1, _i("Service type"), [new RequiredFormInputValidator()], self::getShopServiceTypes());

        return $input;
    }

    public static function getUserIdInput($table_name, $model) : AbstractFormInput
    {
        return new HiddenInput($table_name, 'user_id', $model->user_id ?? Auth::user()->id ?? null, "", []);
    }

    public static function getUsersAutocompleteInput($label, $value)
    {
        $autocomplete = new AutocompleteInput('users', 'full_name', $value, '<span class="">'. $label.'</span>', [], route("core_user_clients.autocomplete") . "?source=agency_app");
    ...
    public static function needValidation()
    {
        return UrlHelper::currentUrlContains('user-profile');
    }

    public static function hideInputAddons($column_name)
    {
        if(!in_array($column_name, ['facebook_url', 'instagram_url', 'tiktok_url','linkedin_url' , 'viber_phone', 'whatsapp_phone', 'mobile', 'telephone', 'email']))
            return true;
    ...
    public static function getTagsInputRecipients($input, $column)
    {

        $recipients_complex = isset($input[$column]) && $input[$column] != null ? json_decode($input[$column], true) : [];

        $user_ids = [];

      //  dd($recipients_complex);

        foreach ($recipients_complex as $recipient)
        {

            if(TextHelper::stringContains($recipient['value'], [' - ']))
            {
                $exploded = explode(" - ", $recipient['value']);

                $user_id = trim($exploded[0]);
